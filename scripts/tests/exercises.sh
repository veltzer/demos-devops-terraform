...
if [ ! -d "./workspace/${username}/${exercise_number}" ]
then
...
function common_exec() {
	if ! docker run --name "${username}-tests" --rm -e "AWS_DEFAULT_REGION=${region}" -e "AWS_ACCESS_KEY_ID=${access_key}" -e "AWS_SECRET_ACCESS_KEY=${secret_key}" -e "TF_VAR_student_alias=${username}" -v "${PWD}:/workdir" terraform-workshop/terraform:latest sh -c "$@"
	then
		touch ../errored
		exit 1
	fi
